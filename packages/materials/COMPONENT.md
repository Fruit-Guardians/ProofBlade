...
```json component-metadata
{
  ...
  "updatedAt": "2026-09-20T12:40:00.000Z",
  "qualityAudit": {
    "bugAuditCount": 32,
    "securityAuditCount": 32,
    "lastBugAuditAt": "2026-09-20T12:40:00.000Z",
    "lastSecurityAuditAt": "2026-09-20T12:40:00.000Z",
    "sourceHash": "c586799f259a8e9a361ed13f8d5ebe7bab54dfac4feec61334d19a610741024a",
    "result": "passed"
  }
}
```
...
- Capability Discovery 以固定代理搜索逻辑 capability/operation，并按请求返回首选/备用 Backend、不可用原因和可选完整 Schema；目录查询不得产生 Effect。Coding Agent 与 Solver 共用同一 Router，真实调用仍经过参数校验、Effect Journal、Artifact 和私有路径保护。
...
- Tool 断路器必须同时覆盖重复失败和无信息增益的成功观察。无进展判定只统计 Tool Contract 明确声明为只读且无副作用、并在滚动窗口内返回相同内容哈希的操作；read-window 终止允许同批成功的普通 process 取消，声明无进展的 declared-no-progress-window 仍要求显式 durableProgress 或真实 durable effect，未解析策略在 read-window 中按潜在进展处理，后续 read-window 不得降级已请求的 declared-no-progress 终止。恢复原因只能在 Harness 确认终止后进入 Agent 返回值和持久化 Assistant 消息，不能误标正常完成或向 GUI 返回空文本。
