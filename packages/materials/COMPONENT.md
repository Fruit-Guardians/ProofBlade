...
```json component-metadata
{
  ...
  "qualityAudit": {
    ...
    "sourceHash": "202056c2bf3a388350cdca6a570bde6e9b5ea20228cbdd4c6e156b7cdfb39b05",
    "result": "passed"
  }
}
```
...
- Tool 断路器必须同时覆盖重复失败和无信息增益的成功观察。无进展判定只统计 Tool Contract 明确声明为只读且无副作用、并在滚动窗口内返回相同内容哈希的操作；read-window 终止允许同批成功的普通 process 取消，声明无进展的 declared-no-progress-window 仍要求显式 durableProgress 或真实 durable effect，未解析策略在 read-window 中按潜在进展处理。恢复原因只能在 Harness 确认终止后进入 Agent 返回值和持久化 Assistant 消息，不能误标正常完成或向 GUI 返回空文本。
