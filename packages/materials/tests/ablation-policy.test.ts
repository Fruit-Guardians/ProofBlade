...
test("defaults cognitive scaffolding to advice while retaining hard gates as an explicit variant", () => {
...
test("hard violations take precedence over soft advice and disabled breakers remain auditable", () => {
  const controller = new AblationPolicyController({ ...DEFAULT_HARNESS_POLICY, firstAction: "soft_advice", duplicateFailure: "hard_stop" });
