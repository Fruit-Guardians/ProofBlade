...
```json component-metadata
{
  ...
  "qualityAudit": {
    ...
    "sourceHash": "41cc4de597ee5f31d72005bafdec6cce542d2d8b8a3b0231dc7429a6ffc50936",
    "result": "passed"
  }
}
```
...
- `version.ts` 的 `createCachedRunVersionSnapshot()` 为每个进程提供一份 revision 键控的版本快照：`createRun` 原本每次都要重扫 Skills、`.mcp.json` 与 tool catalog 并重算四个 catalog hash，而这些输入不变时快照也不变。**revision 由内容推导：每次调用都读取并哈希每个输入的字节，没有任何以元数据为键的摘要缓存。** 这一点是实测取舍而非简化——`path|ino|size|mtimeMs|ctimeMs` 这类键仍能自我复现（保留字节长度、用 `utimes` 复位 mtime，只余 ino/ctime 作证据，而两者都不是内容），继续加宽键只是移动窗口；而带键缓存每次净省约 2.6ms（检查 ~6.7ms vs 重建 ~9.3ms），单遍读取+哈希为 ~11.8ms，仍在重建成本之下。config 的 `piVersion` 与 `thinkingLevel` 直接进摘要，因为 builder 读的是解析后的 config 对象而非配置文件（GUI 启动会替换该对象）。构建失败的 promise 绝不入缓存，避免一次瞬时读失败变成永久失败。缓存是透明的：`createCachedRunVersionSnapshot()` 的返回值必须与直接 `createRunVersionSnapshot()` 深度相等。
