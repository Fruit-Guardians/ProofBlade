...
function preparedChallengeProfileBlock(profile: ChallengeToolProfile, preflight?: ChallengeToolPreflight): string {
  ...
  const reverseProgression = profile.id === "reverse"
    ? " Reverse progression: after one successful metadata call for the current binary, move to a target-relevant function list, decompilation, or disassembly. If the decompiler dependency is unavailable, call binary_disassemble with a fixture-relative path and a known address. For a stripped ELF, use its entry point or a discovered address window; `objdump --disassemble=main` has no symbol to resolve. Do not repeat metadata unless the target or binary changes."
    : "";
...
export function codingCtfCategoryGuidance(kind?: TaskContract["target_kind"], target?: string, pwnToolsAvailable?: boolean, pwnReproductionAvailable = pwnToolsAvailable, webToolsAvailable?: boolean): string {
  ...
  if (kind === "reverse") {
    return [
      ...
      "- Prefer a decompiler MCP (idalib-mcp / jadx-mcp when available) over hand-reading `objdump -d`. Open the target once, then work from pseudocode. If its GUI dependency is unavailable, call `binary_disassemble` with a fixture-relative path and known address; for a stripped ELF use the entry point or discovered address window, never `objdump --disassemble=main`.",
