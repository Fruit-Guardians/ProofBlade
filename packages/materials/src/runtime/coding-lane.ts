...
When the user asks for a CTF flag, challenge answer, recovered secret, or another deterministic result from workspace evidence, inspect the real inputs and test decoy hypotheses against file structures and control flow. Before reporting a final candidate as confirmed, call verify_claim with the exact candidate and a deterministic command that derives and prints it without embedding the candidate literal. Supporting evidence ids are optional; do not delay a verifier-ready candidate merely to manually curate evidence. Treat strings output alone as an observation, not verification. If reproduction is still missing, state that the conclusion is unverified and name the missing check.
...
function preparedChallengeProfileBlock(profile: ChallengeToolProfile, preflight?: ChallengeToolPreflight): string {
  ...
  const firstActionBudget = ` Suggested opening probe: prefer ${firstAction.allowedToolNames.join(", ")} for the first ${firstAction.maxCalls} calls; completion tools may be used immediately when a verifier-ready candidate exists. You may deviate when a concrete observation makes another in-scope tool more informative, and should record why.`;
  ...
  const reverseProgression = profile.id === "reverse"
    ? " Reverse progression: after one successful metadata call for the current binary, move to a target-relevant function list, decompilation, or disassembly. If the decompiler dependency is unavailable, use capability proofblade.binary.disassemble with a fixture-relative path and a known address. For a stripped ELF, use its entry point or a discovered address window; `objdump --disassemble=main` has no symbol to resolve. Do not repeat metadata unless the target or binary changes."
    : "";
...
export function codingCtfCategoryGuidance(kind?: TaskContract["target_kind"], target?: string, pwnToolsAvailable?: boolean, pwnReproductionAvailable = pwnToolsAvailable, webToolsAvailable?: boolean): string {
  ...
  if (kind === "reverse") {
    return [
      ...
      "- Prefer a decompiler MCP (idalib-mcp / jadx-mcp when available) over hand-reading `objdump -d`. Open the target once, then work from pseudocode. If its GUI dependency is unavailable, use `capability` to invoke `proofblade.binary.disassemble` with a fixture-relative path and known address; for a stripped ELF use the entry point or discovered address window, never `objdump --disassemble=main`.",
