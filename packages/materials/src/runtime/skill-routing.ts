...
const SIGNALS: Array<{ skill: string; score: number; reason: string; pattern: RegExp; corroborating?: boolean }> = [
  ...
  // The generic bucket never selects on its own. It fires on any task that says
  // "CTF" or 题目, so selecting with it injects a whole skill -- pyjail, bash jail,
  // HISTFILE, high-numbered SECCOMP fds, rvim escapes -- into tasks that have
  // nothing to do with it. CHAT-1790096643438 was a firmware patch chain plus key
  // derivation and got exactly that. It is a corroborating signal now: it can ride
  // along with a domain signal, and it abstains when it is the only evidence.
  { skill: "ctf-misc", score: 12, reason: "generic CTF or puzzle signal", corroborating: true, pattern: /\b(?:ctf|challenge|pyjail|encoding|unicode|qr|audio|constraint|题目|解题|杂项)\b/i },
];
...
export function routeSkillsForTask(task: RoutingTask, availableSkillNames: Iterable<string>, maxSkills = 2): SkillRoute[] {
  ...
  let corroboratingMatched = false;
  for (const signal of SIGNALS) {
    if (!signal.pattern.test(haystack)) continue;
    if (signal.corroborating) {
      corroboratingMatched = true;
      continue;
    }
    add(signal.skill, signal.score, signal.reason);
  }
  // Abstain rather than guess: no domain prior, no specific signal, only the
  // generic bucket (or nothing at all). Returning no skills leaves the model with
  // its own context instead of a plausible-looking but unrelated skill.
  if (scores.size === 0) return [];
  if (corroboratingMatched) add("ctf-misc", 12, "generic CTF or puzzle signal (corroborates a domain signal; never selects alone)");
