...
/** Render known MCP environment denials as a recovery instruction, not a bare remote error. */
export function mcpToolErrorMessage(server: string, tool: string, result: { content?: unknown }): string {
  ...
  if (/idalib/i.test(server) && /disassemble/i.test(tool) && /can't import pyside6|qt without gui/i.test(detail)) {
    return `[ProofBlade MCP tool blocked: ${server}.${tool}]\nReason: this IDALIB instance lacks the GUI/Qt dependency required by its static-analysis path. ${prefix}. ${detail}\nThe requested static-analysis action was not executed, and no code fact was obtained.\nNext: call capability search with query "binary disassemble", then call capability with { operation:"invoke", capabilityId:"proofblade.binary", capabilityOperation:"disassemble", input:{ path:"<fixture-relative binary>", address:"<same address>", maxInstructions:128 } }; or install a headless-capable IDALIB environment. Do not repeat this IDALIB code-analysis call until that dependency is available.`;
  }
  if (/idalib/i.test(server) && /decompile/i.test(tool) && /can't import pyside6|qt without gui/i.test(detail)) {
    return `[ProofBlade MCP tool blocked: ${server}.${tool}]\nReason: this IDALIB instance lacks the GUI/Qt dependency required by its decompiler. ${prefix}. ${detail}\nThe requested decompilation action was not executed, and no code fact was obtained.\nNext: make at most one disassemble_function probe on the same address to check whether this endpoint's static-analysis path is usable; if it reports the same dependency error, call capability search with query "binary disassemble", then call capability with { operation:"invoke", capabilityId:"proofblade.binary", capabilityOperation:"disassemble", input:{ path:"<fixture-relative binary>", address:"<same address>", maxInstructions:128 } }; or install a headless-capable IDALIB environment. Do not repeat decompile_function until that dependency is available.`;
  }
