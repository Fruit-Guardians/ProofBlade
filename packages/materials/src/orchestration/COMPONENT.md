...
- `SingleAgentLoop` 在 lane 建成后通过 `onLaneReady` 暴露运行控制句柄；每轮模型调用前后都必须检查 durable `PAUSED` 状态。所有任务都从这个通用入口执行，历史任务由只读迁移适配器投影。
- Loop 的 `onEvent` 会沿着统一 lane factory 传递到 Pi Harness，使 GUI 续解与首轮一样能实时显示 tool start/end、Provider 文本和上下文事件；事件展示不改变 Control Store 真相。
