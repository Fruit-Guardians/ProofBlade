...
```json component-metadata
{
  ...
  "qualityAudit": {
    ...
    "sourceHash": "f878f6e941d0bb4d3963bfdbe67b5b8668f16abeb046c4623ec9002c93085137",
    "result": "passed"
  }
}
```
...
- `baseline-v4` 的最小基线是 37 项：六题各三次形成 18 个生产循环，另有 19 个运行时场景；场景包含 Job monitor 到观察队列的完成、重建和幂等确认；全部成功、全部证据绑定、全部重放一致、全部事实证据覆盖且无候选泄漏。
- 18 个运行时场景固定覆盖缓存用量与前缀漂移、上下文单调性与用户任务锚点、三种 Tool 收敛断路器、Evidence 整理背压与并发去重、暂停重放、Verifier 权限和 Lease 所有权隔离，以及事件 ingress 的优先级/幂等/generation fencing/coalescing、RequestEpoch 重建和脱敏、生命周期孤儿审计、Scope 释放、Spill 失败回退与单 Agent 能力门禁。
...
新增能力必须先判断信息位置：题目求解能力增加 Fixture；跨题的缓存、上下文、调度、证据、恢复或权限不变量增加 Runtime Scenario。保持结果机器可读、顺序稳定并记录 attempt/runId。开发时允许子集报告，合并前必须执行完整 37 项门禁。
