...
- `model-context-frame.ts` 在 Provider adapter 完成后记录 metadata-only 的最终模型可见消息、来源哈希、字符/Token 统计和 omitted 项；它不保存 prompt、候选或正文。
...
每个 Provider 请求的最终 payload 都必须能够重建一个 `ModelContextFrame`。Frame 记录“进入最终请求”的消息与“被省略”的项目，使用 content hash 和来源引用而不是复制敏感正文；RequestEpoch 只保存 frame ID/hash，完整原始消息仍由 Provider/Session 边界管理。
