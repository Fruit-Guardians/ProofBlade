...
function Overview({ detail }: { detail: RunDetail }) {
  ...
  return <div className="overview-page">
    ...
    <section><div className="section-head"><div><strong>控制面</strong><span>{control.domainPhase}</span></div><StatusMini status={control.gate.status} /></div><dl className="key-values"><dt>门禁缺口</dt><dd>{control.gate.missing.join(", ") || "无"}{control.gate.stale.length > 0 ? ` · stale: ${control.gate.stale.join(", ")}` : ""}</dd><dt>阶段动作预算</dt><dd>{control.budget.phaseActionsUsed} / {control.budget.phaseActionsRemaining === undefined ? "--" : control.budget.phaseActionsUsed + control.budget.phaseActionsRemaining}</dd><dt>Journaled Effect 预算</dt><dd>{control.budget.journaledEffectsUsed} used · {control.budget.journaledEffectsRemaining} remaining<small>（只统计进入 Effect Journal 的调用；bash/read/edit 等不计入，因此它不是工具调用次数）</small></dd><dt>提交预算</dt><dd>{control.budget.submissionsUsed} used · {control.budget.submissionsRemaining} remaining</dd><dt>恢复预算</dt><dd>{control.budget.replansUsed} / {control.budget.replanLimit} · 剩余 {control.budget.replansRemaining}</dd><dt>Verifier 恢复</dt><dd>{control.recovery.required > 0 ? `${control.recovery.required} 个请求等待受控恢复` : "无待恢复请求"}</dd><dt>下一动作</dt><dd>{control.nextAction ? `${control.nextAction.objective} · ${control.nextAction.toolNames.join(", ") || "无预备工具"} · ${control.nextAction.maxCalls} 次` : "当前阶段没有预备动作"}</dd></dl>{control.recovery.items.length > 0 && <div className="ledger-lines">{control.recovery.items.map((item) => <div key={item.requestId}><StatusMini status={item.state} /><code>{item.kind}</code><span>{item.reason ?? item.requestId}</span></div>)}</div>}</section>
