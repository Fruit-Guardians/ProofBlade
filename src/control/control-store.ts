...
export class ControlStore {
  ...
  public async dispatch(runId: string, command: DomainCommand): Promise<HarnessEvent[]> {
    ...
    const after = await this.eventStore.replay(runId);
    await this.eventStore.saveProjection(after);
    return events;
  }

  public async append(runId: string, events: Array<Omit<HarnessEvent, "seq" | "id" | "runId" | "ts">>): Promise<void> {
    ...
    await this.eventStore.saveProjection(await this.eventStore.replay(runId));
  }
...
export function createEffectInput(runId: string, tool: string, args: Record<string, unknown>, replayPolicy: ReplayPolicy, generation: number): { effectId: string; idempotencyKey: string } {
  const normalizedArgs = canonicalJson(args);
  return { effectId: id("EF"), idempotencyKey: sha256(`${runId}:${tool}:${normalizedArgs}:${generation}:${replayPolicy}`) };
}
