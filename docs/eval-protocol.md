...
The machine-readable gate requires at least three attempts, exact coverage of all six fixture profiles, 100% success, evidence-backed success, replay parity and confirmed-Fact evidence coverage, plus zero candidate leaks. The report hash excludes run ids, wall-clock timing and raw error text so equivalent runs can be compared across machines. It retains behavioral counters, gate results and stable per-case outcomes.
