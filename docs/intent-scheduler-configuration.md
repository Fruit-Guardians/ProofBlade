...
### 1. 查看为什么没有触发调度
```typescript
const context = await controlStore.getSchedulingContext(runId);
console.log('触发条件检查:');
console.log('  新增 Fact:', context.newHighValueFacts > 0 ? '✅' : '❌');
console.log('  Intent 归零:', context.openIntents === 0 ? '✅' : '❌');
console.log('  连续失败:', context.consecutiveFailures >= 2 ? '✅' : '❌');
console.log('  预算过半:', context.phaseBudgetUsed > 0.5 ? '✅' : '❌');
console.log('  新 Hint:', context.newHints.length > 0 ? '✅' : '❌');
console.log('  Verifier 否决:', context.verifierRejected ? '✅' : '❌');
```

### 2. 查看过滤统计
```typescript
const filter = new IntentFilter(leaseManager);
