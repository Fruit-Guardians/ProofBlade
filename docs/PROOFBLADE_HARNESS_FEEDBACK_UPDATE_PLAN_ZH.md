...
- **第一份已核实为该 run 的最终 assistant 消息**：`runs/CHAT-1790096643438/events.jsonl` 事件 **#355** 的 `text` 与之逐字一致（该 run 共 362 条事件，工具结果 10 次）。
- **第二份来自哪个对话尚未确认**（所有者指出它并非出自该 run）。它与第一份同指这批 id——`VR-1e0de02d…`、`VR-de190718…`、`C-af331919…`、`sha256=ee47119c…`、`TREE-b8f448bb`、`TREE-6254a5fe`——差别只在结尾少了「最后一句公道话」那一段。

两份描述的显然是同一个 run，因此本文按同一条目处理。**在第二份的来源得到确认之前，它不作为独立证据**；下文每条的核对对象都是代码与该 run 的事件。
...
## 5. 落地状态（2026-09-23 更新）

所有者确认后，本计划全部条目已实现并各自提交 PR，**一律未合并**（要求先评审）。每条都带变异验证；实现细节与验证输出见各 PR 描述与账本 `UPDATE-20260923-004` … `-010`。

| 计划项 | 状态 | PR | 变异验证 |
|---|---|---|---|
| P0-1 预算字段按实际计量改名 | 已提 PR | #250 | 键名改回旧名 → `context.test.ts` 转红 |
| P0-1 真实工具调用计数 | 已提 PR | #255 | reducer 自增改回 no-op → 新用例转红 |
| P0-2 skill 路由无信号时弃权 | 已提 PR | #251 | 去掉 `corroborating` → 两条新用例转红 |
| P0-3 无 verifier 任务 observed-only | 已提 PR | #252 | 守卫改恒假 → 新用例转红 |
| P0-3 同一结论不建第二棵树 | 已提 PR | #254 | 交集判定改恒不命中 → 新用例转红 |
| P1-1 phase/gate 由真实证据推进（RECON） | 已提 PR | #253 | 改回只看假设 → 新用例转红 |
| P1-2 重复探测判定需真实重复 | 已提 PR | #256 | 重复判定改恒真 → 新用例转红 |
| 本文档 | 已提 PR | #249 | — |

### 5.1 实现过程中被测量修正的两处

- **§1 第 2 条的「未复现」改为「成立」**：该 run 的 15 条 `experiment_recorded` 有 **15 个互不相同的 `repeatKey`**，而模型收到的是 `{count:4, key:'long-running', reason:'long_running'}`。计数来自 `isLongRunningCommand` 对**命令文本**的启发式（含 `for`/`while`、`timeout N`、fuzzer 名），与是否重复无关——所以误判成立，根因就是它。P1-2 因此按「要求真实重复」实现，而不是改文案。
- **§1 第 9 条的重叠机制写得更准**：两棵树共享的**不是**同一个候选 artifact（completion 自己的是 `A-7e777dd3`），而是 supporting Evidence `EV-3a67f1b4` 及其指向的 `A-76a42d21`。所以 P0-3 的判据取「本次要挂的节点与既有树有交集」，而不是「同一个候选」——后者实测不会命中。

### 5.2 仍然只做了度量、没有改代码的一项

- **§1 第 4 条「注入了一大段无关技能正文」仍未复现**：`ctf-misc` 确实出现在该 run 的 prompt 快照里，但 pyjail / HISTFILE / SECCOMP / rvim / Levenshtein 这些正文在快照中零命中。P0-2 因此只按「兜底规则过宽」落地（规则本身已核实），注入体量需要一次带 `load_skill` 的复现才能定性。
