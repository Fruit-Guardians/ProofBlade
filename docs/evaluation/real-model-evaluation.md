...
`eval-real` compares two or more distinct configured provider profiles on one private local corpus. It never runs in CI. Use `--preflight` first to validate the corpus, Web/Pwn coverage, priced variants, credential environment variables, and budget without creating a Run or sending a Provider request; the actual comparison requires `--allow-live`. Two variant ids that resolve to the same provider/model/endpoint profile are rejected, because they do not measure a meaningful model comparison.

The profile fingerprint includes the provider API, endpoint, model, endpoint mode, proxy, and declared credential environment-variable identity (never the secret value). The CLI's live gate also requires observable Provider traffic from every Variant, at least 20 corpus cases, and both Web and Pwn target kinds. A deterministic injected lane, a tiny corpus, or a run that fails before the first Provider request cannot satisfy `eval-real --enforce-gate`, even if it produces a locally accepted candidate. The provider-free local holdout explicitly disables these checks because its purpose is to exercise Run/verifier/replay plumbing.
...
Ready-to-copy config skeletons are available at `examples/real-evaluation-provider.openai.example.json`, `examples/real-evaluation-provider.anthropic.example.json`, `examples/real-evaluation-provider.aihub.example.json`, `examples/real-evaluation-provider.aihub-luna-max.example.json`, and `examples/real-evaluation-provider.aihub-terra-max.example.json`. The Luna max and Terra max examples are preconfigured for AIHub's OpenAI Responses endpoint with reasoning enabled; they use `gpt-5.6-luna` and `gpt-5.6-terra` respectively, both with `thinkingLevel: "max"`. Verify current pricing before a live run. For a single-key paired comparison, use `examples/real-evaluation-provider.deepseek-flash.example.json` and `examples/real-evaluation-provider.deepseek-pro.example.json`; both reference `PROOFBLADE_EVAL_DEEPSEEK_API_KEY` and the official OpenAI-compatible DeepSeek endpoint. The DeepSeek examples use the currently published peak rates as a conservative reservation (cache miss for input, cache hit for `cacheRead`, and zero `cacheWrite` because the published table has no separate write rate). The aihub example assumes an OpenAI-compatible `/v1` base URL, uses `PROOFBLADE_EVAL_AIHUB_API_KEY`, and intentionally leaves the model name and pricing as values to verify against the aihub account. Copy the selected files into the ignored `.proofblade/evaluation/` directory, replace the model names only if the Provider has changed them, verify the currently published prices, and export the referenced environment variables. Prices can change and the live run must be rechecked against the Provider's current pricing page.
...
For the current local Web/Pwn runs, see the sanitized [2026-08-24 evaluation snapshot](./real-web-pwn-20260824-results.md). It is an aggregate review artifact, not a replacement for rerunning a fixed private corpus with the current model and budget.
