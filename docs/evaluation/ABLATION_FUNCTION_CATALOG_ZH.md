...
`ablation report --results <real-model-summary.json> --markdown` 现在可直接读取真实评测 summary，生成同一份闭环报告：原始 Pass 指标、可比较分母、Provider/预算/环境污染排除原因、失败责任域、成功轨迹的首工具/`verify_claim`/Evidence/首证据时间，以及根据首错生成的下一步修复动作。原始成功率始终保留；污染排除只用于策略因果比较，不能删除或掩盖失败记录。
