...
2. **真正剩余的条目是"读取路径的 O(历史) 输入"。** `control-store.ts:736` 在读取时**总是先取回并解析整条事件流**（投影只有 4.5 KB，20,200 事件下仍是 4.5 KB，即投影是 O(状态) 的而读取路径不是）。若读取只解析 `(投影 lastSeq, 流末]` 这一段，冷读与屏障都能降为 O(增量)。这是一条独立条目，不属于 PLAN-240 现有范围。
...
**关于"几毫秒被放大到秒级"这条**：机制已定位并**更正**——不是重放回退，而是读取路径无条件解析整条事件流（`control-store.ts:736`），详见 `docs/PROOFBLADE_TOOL_HOT_PATH_COST_BREAKDOWN_ZH.md` §6.3/§6.4。**修法是把封印改成链式哈希以支持 O(增量) 读取，属于事件/投影协议的破坏性变更，需要单独决策，未包含在上述任何 PR 中。**
