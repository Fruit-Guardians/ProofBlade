...
更新时间：2026-09-22（评审修订 3：数字口径对齐 + 撤销的 O(增量) 读取快路径）

> **数字口径**：本计划引用的所有毫秒值来自**单机**测量（Windows / i9-14900HX / Node 22），可复现入口是 `npm run baseline:tools` / `baseline:tools:real` / `baseline:tools:longrun` 三个。nearest-rank 下 `n=8` 的 `p95` **就是最大值**；凡未经入库 harness 产生的数字都标了来源与置信度。

> **修订说明（评审修订 3）**
>
> 第三、四轮评审指出本节把「机制推演」当成了「已交付」，具体有四处：①§7.2.1 的两行端到端数字（211.6ms / 2095.8ms）来自一个未入库、未记录 `lastSeq` 的一次性探针，却被当成基线；②同一组三个数字（1647.9 / 223.3 / 1846.0）被写了两遍；③`control-store.ts:736` 这类行号引用已随注释增删失效；④摘要表里「read ≈ 25ms **固定**」与正文已更正的口径（25ms 是 `command` 列、含它触发的两次 durable 提交）自相矛盾。四处均已**就地替换**而非追加：
>
> 1. §7.2.1 把两行探针数字标为**历史记录、不可复现、非受控对照**（2095.8ms 与可复现的 1846.0ms 名义条件相同，但事件数差 1 解释不了 250ms 的差），并明确以可复现的三点时序为准；重复的那段合并为一处。
> 2. 行号引用换成函数与调用名（`#readSnapshot` 的 `eventStore.events(runId)`）。
> 3. 摘要表两行改为实际口径：`read ≈ 25ms` 注明是 `command` 列且含两次 durable 提交；9.3s 那行标明是 `replay()`（n=1）**最后一道回退**，不是冷读路径。
> 4. §7.2.2 的「读取路径降为 O(增量)」补上了**它已被撤销**的事实与原因：封印无法见证历史前缀字节，同长度就地改写会让快路径与 `replay()` 对同一次 Run 给出不同答案。撤销记录同时写在账本 `UPDATE-20260920-002` 自己的条目里（该条目原先把它记成已交付）。
>
> **本修订没有重新测量任何数字**：新增的都是把已有实测的来源、边界与撤销状态写清楚。凡本文件与本仓库账本冲突处，以账本与其指向的测试为准。

> **修订说明（评审修订 2）**
>
> 1. **§7.2 的中心论断是「用户报告的可观察现象」，不是本仓库的实测结论**。「几毫秒的命令被外围链路放大到秒级」来自使用者在真实环境的口头反馈（症状：长对话下每次操作都要等数秒；日期见本节「修订说明」），**没有**附日志、机器信息或复现步骤。它足以作为优先级的输入，不足以作为「已确认事实」。承担优先级论证的是 T0 基线（PR #230 / #240），它把问题定位到**读取路径**而不是热路径：热路径实测 25–37ms 平坦，秒级出现在读取与投影重建上。基线的用途仍是量化放大倍数、定位主要成本来源、作为改进对比基准。
...
#### 5.6.2 引入 `ToolResultCommit`（**已重开：原「不可行」判定被实测推翻**）

> **评审修订 4（实测复核，取代修订 3）**：修订 3 判定本节不可行，理由是 `ControlStore.dispatchTransaction` 用**批次之前的快照**校验整批命令。**该理由与代码不符**：`#commitCommands` 是逐条折叠的——
>
> ```ts
> let after = before;
> const references = buildBatchReferences(before, commands);   // 已并入本批次将创建的 id
> for (const command of commands) {
>   validateCommand(after, command, references, authority);    // 校验的是折叠后的 after
>   after = reduce(after, event);
> }
> ```
>
> 因此「注册 artifact 并在同一批次注解/引用它」是**现有事务模型的既有能力**，不是语义变更：
>
> - `artifact_annotation` 查 `after.artifacts`，同批 `artifact` 在前即可；
> - `observation` / `domain_record` 查 `references`，其中已含同批创建的 artifact id，**顺序无关**；
> - `evidence` 在无 `effectId` 时提前返回，同样不构成阻断；
> - 实测 3 条用例见 `packages/materials/tests/dispatch-transaction-batch.test.ts`（四条命令一个事务全部落盘 / 前向引用成立 / 批内谁都没创建的 id 仍被 `artifact_annotation` 原子拒绝）。
>
> **表项 T2 因此重开为 P0**：合并路径不需要改事务模型，也不需要引入延后队列与其屏障。§5.6.3 的 `<= 1` 目标可由本路径达成。详见 `docs/PROOFBLADE_TOOL_HOT_PATH_COST_BREAKDOWN_ZH.md` §2.5 / §2.6。
>
> 下文保留原始设计，作为重新实施的起点。
...
1. **每次调用的耗时几乎不随载荷变化，固定部分在 `command` 列内部。** `read-1B` 与 `read-32KiB` 事件数完全相同（各 4 条），但延迟从 25.1ms 增长到 43.0ms——增量来自 32KiB 的归档 I/O。
   这 25ms 是 `command` 列（工具体本身）的耗时，其中**包含**它触发的两次 durable 提交；`wrapper − command` 只有 1.4–4ms。所以它不是「附加在命令之外的链路开销」，而是「命令内部含它自己的两次提交」——本条早期版本写成「控制链路每次调用是固定成本，约 25ms」并以粗体给出，那个表述是错的，已就地替换。
...
2. **`replay()` 随历史线性增长，10,000 事件约 9.3 秒（n=1）。** `replay()` 按设计折叠整条事件流，慢是应该的；它位于读取的**最后**一道回退上：`control-store.ts` 在 `loadProjection` 无法返回有效投影时才回退到 `eventStore.replay()`。
   **归因更正（就地写全，因为它原先的更正只存在于一个未合并的分支上）**：秒级读数**不是**这条回退造成的。PR #244 的分解实验表明，投影陈旧时 `snapshot()` 走的是「投影 + `applyTail`」（该计数器为一次性探针，已随 PR #244 的改动一并还原；结论基于当时的分解实验，现无入库 harness 可复现），真正的秒级来自读取路径**无条件先取回并解析整条事件流**，然后才用投影做增量折叠。因此下面第 2 条「建议新增 P0」的修法目标也随之改变：要修的是**读取路径的 O(历史) 输入**，不是回退代价。完整证据链见拆分文档 §6.3–§6.5。
...
- **建议新增一项 P0：长 Run 的读取路径必须有界或可避免**（例如读取只解析投影 `lastSeq` 之后的尾部，前缀校验按 seal 的前缀哈希完成）。本计划原本没有这一项，是本次实测暴露的。

**回退代价的端到端实测**（10,000 事件，走真实 GUI 读取路径 `DebugDataService.getRun`，冷缓存 = 重启后的 GUI）。**下表两行来自一个未入库的一次性探针，只能当历史记录读，不能当基线**（原因见下）：

| 条件 | projection.json | getRun | 该投影的 lastSeq |
|---|---:|---:|---:|
| 冷缓存，投影存在 | 4.6 KB | **211.6ms** | **1**（见下方更正） |
| 冷缓存，投影**被删除** | absent | **2095.8ms**（同一探针；可复现值 **1846.0ms**，见下） | — |

**上表为什么不能当基线**，三点分开写，因为它们的性质不同：①「4.6 KB」与本页 §7.2 的「28.7 KB」不是同一次运行。28.7 KB 是 10,000 事件长 Run 的当前投影，而 4.6 KB 那一份封在**很早就结束的 seq 上**（复测直接读到 `lastSeq = 1`），所以 211.6ms 那一格不是「投影陈旧但仍可用」，而接近「投影几乎为空」；「一个 4.6KB 的文件决定读取是 0.21s 还是 2.1s」把**文件大小**当成了自变量，真正的自变量是**投影的时效**，同一份 4.6KB 文件在两种时效下并不等价。②该探针**没有记录**每组的 `lastSeq`、屏障状态与是否冷缓存，也没有入库，所以两行都**不可复现**。③因此 2095.8ms 与下表的 1846.0ms 虽然名义条件相同（投影被删除），**不是受控对照**：事件数差 1（10,000 vs 10,001）解释不了 250ms 的差，探针本身也无法重跑。**以可复现的那组为准，2095.8ms 只作为历史记录保留。**

**可复现的三点时序**（同一路径、10,001 事件、探针已入库，拆分文档 §6.4 / §6.5）：投影陈旧 **1647.9ms**、回合边界屏障后 **223.3ms**、投影被删除 **1846.0ms**。其中「投影陈旧」这一档（快速路径刚写完、屏障尚未补投影）才是使用者实际遇到的时序，也正是上表缺的那一档。

**但分解实验表明秒级并非回退造成**：投影陈旧时 `snapshot()` 走的是「投影 + `applyTail`」而非 `eventStore.replay()`（该计数器为一次性探针，已随 PR #244 的改动一并还原；结论基于当时的分解实验，现无入库 harness 可复现）。同一形态下 `loadProjection()` 只要 **49.1ms**、`snapshot()` 却要 **1877.3ms**，差额全部来自 `#readSnapshot` 里那句 `eventStore.events(runId)` —— 它**无条件取回并解析整条事件流**，然后才用投影做增量折叠（引用函数名而不是行号：该调用在本次评审期间已因注释增删换过三次位置）。所以上文「重放回退」是**推断而非实测**，正确的机制是 §6.3 的「读取路径输入是 O(历史)」；屏障的收益则是让 GUI 详情走 `loadProjectionHint()`（0.4ms）短路。修这一条即可同时消除秒级与残余窗口 —— **但下面 §7.2.2 的「快速路径」在落地后又因封印无法见证历史前缀而被撤销：读取路径仍付 O(历史)，这是设计决定，不是遗留缺陷。**

这里有一个**必须点明的反直觉后果**：T1/T2/T3 一路把 `persistProjection: false` 铺到热路径上，换来了快速调用；代价是 **`projection.json` 会长时间停留在陈旧状态**。于是"投影缺失或陈旧"不再是罕见故障，而是**正常运行下会出现的状态**——一旦缓存冷掉，读取就要付出 O(历史) 的代价（**机制见上文更正：是先解析整条事件流，不是全量重放**）。

也就是说：**投影延迟写入的收益与读取退化的风险是同一枚硬币的两面。** 新 P0 应当同时考虑"在明确屏障处补齐投影"，而不是只优化退化本身。

**该方向已验证，但收益被高估，必须连单价一起引用**（拆分文档 §6.3，本机实测）：屏障点确实存在且**天然落在回合边界**（`pi-events.ts:524` 的 `turn_end`/`agent_end`，以及 `coding-lane.ts:879` 的通道收尾）。10,000 事件下，不设屏障时冷读 1524 ms；设 1 次屏障后降到 1168 ms，之后加密到每 500 事件一次也只降到 569 ms——**单次屏障约 113–165 ms 且与间隔无关，屏障累计开销随次数线性上涨**。也就是说**屏障并没有消除这笔 O(历史) 成本，只是把它从读取路径前移到回合边界**。

因此本条的落地口径修正为两条：

1. **维持回合边界屏障，不要提高屏障频率。** 收益的绝大部分由第一次屏障拿到，加密只买到递减的几十到几百毫秒，却按次付费。
2. **真正剩余的条目是"读取路径的 O(历史) 输入"。** 读取路径（`#readSnapshot`，其 `eventStore.events(runId)` 调用）**总是先取回并解析整条事件流**（投影只有 4.5 KB，20,200 事件下仍是 4.5 KB，即投影是 O(状态) 的而读取路径不是）。若读取只解析 `(投影 lastSeq, 流末]` 这一段，冷读与屏障都能降为 O(增量)。这是一条独立条目，不属于 PLAN-240 现有范围。
   **已经试过一次，且被撤销**（`fix/barrier-cadence-evidence` 的 `UPDATE-20260920-002`，撤销记录就在那条账本条目自身里）：投影当前时的快路径确实做到了 1876ms → 12.5ms，但它把「投影 + 末尾 seq」当成与 `replay()` 等价。封印（`projectionSealPayload()`）里**没有 size 字段**，`eventPrefixHash` 又是对**解析后事件**的规范 JSON 求哈希，所以它证明不了历史字节就是前缀哈希算过的那一份：同长度就地改写一条事件后，快路径返回封印那一刻的状态、`replay()` 折叠改写后的事件，**两者对同一次 Run 给出不同答案**（回归用例：`projection-read-bound.test.ts` 的「snapshot and replay agree after a historical event is rewritten in place」）。要真的降为 O(增量)，前提是把封印换成能见证前缀字节的形式（链式哈希是一类做法），那是事件/投影协议的破坏性变更，须单独决策与迁移方案。**在那之前，读取路径付 O(历史) 是设计决定，不是待修的缺陷。**

| 待测指标 | 基线值 | 状态 |
|---|---:|---|
| 普通对话创建 API p50 / p95 | — | **待测** |
| 底层命令 <= 10 ms 时，普通工具总耗时 p50 / p95 | read ≈ 29–37ms（不随历史变化） | **已测** |
| 普通工具 ProofBlade 附加耗时 p50 / p95 / p99 | read ≈ 25ms（`command` 列，含它触发的两次 durable 提交）；bash ≈ 4ms | **已测局部** |
| 长 Run 单次工具调用 | 10,000 事件下仍 ≈ 37ms | **已测** |
| 长 Run 全量 `replay()`（n=1，读取的最后一道回退，**不是冷读路径**） | 10,000 事件 ≈ 9.3s | **已测（新发现）** |
| 长 Run 冷读（投影陈旧，走投影路径） | 10,000 事件 ≈ 1877ms，其中 `loadProjection` 仅 49ms | **已测（§6.4 分解）** |
| 长 Run 冷读（投影当前，GUI 走 hint 短路） | 10,000 事件 ≈ 248ms | **已测** |
| 重要工具结果单次提交 p95 | — | **待测** |
...
上面 14 条的实际交付情况。**PR 编号与分支的对应关系取自 GitHub，不是按提交顺序推断的。**

| §10 条目 | 交付 PR | 状态 |
|---|---|---|
| PR 1 构建一致性（A） | #222 | ✅ 启动形状断言，无运行时降级 |
| PR 2 计时与基线（T0） | #223、#230、#240 | ✅ provider-free + 真实 Run + 长 Run 三段基线 |
| PR 3 轻量创建（C） | #226 | ✅ |
| PR 4 staging 回归（E'） | #225 | ✅ 双向断言 |
| PR 5 ToolResultEnvelope（D0） | — | ➖ **实测为不必要的抽象层**，未实施 |
| PR 6 live buffer（D1+T1+T3） | #228（T3）、#229、#232 | ⚠️ 部分：commit ≤1 已达标，**"零同步提交"不可达**——Artifact 落盘是 verifier 的引用目标，不能延后（#231 记录） |
| PR 7 批量持久化（T2） | #233 | 🔁 **已重开**：原「实测不可行」不成立（`dispatchTransaction` 按批内折叠后的快照逐条校验，不是批前快照），合并可行；见拆分文档 §2.5 复核与 `dispatch-transaction-batch.test.ts` |
| PR 8 去重 hash 与回读（T4+D3） | #229、#232 | ✅ |
| PR 9 SpillStore（D2） | — | ➖ 实测会引入并行抽象层，未实施 |
| PR 10 缓存 snapshot 与 catalogs（D+F） | #227、#234 | ✅ |
| PR 11 Context 热路径（G） | #235 | ➖ **实测无剩余空间**，已关闭 |
| PR 12 详情拆分与增量轮询（H+I） | #239（I 的服务端谓词） | ⚠️ **未完成**：`afterSeq`/`limit` 服务端已就绪、客户端从未调用；剩余部分需要真机/浏览器验证（§6.2） |
| PR 13 基准与预算门禁（J） | #236 | ✅ 门禁只断言计数 |
| 条件项 deferred version snapshot | — | 未触发 |

**另有 4 项不在这份清单里、由本轮实测新增的工作**：#237（observer 失败不再静默）、#238（归档失败不推翻读取成功语义）、#241/#242/#243（重放回退成本、屏障廉价校验、投影 hint 时效）、#244（读取路径计数 + 秒级归因更正）。

**关于"几毫秒被放大到秒级"这条**：机制已定位并**更正**——不是重放回退，而是读取路径无条件解析整条事件流（`#readSnapshot` 的 `eventStore.events(runId)`），详见 `docs/PROOFBLADE_TOOL_HOT_PATH_COST_BREAKDOWN_ZH.md` §6.3/§6.4。**修法是把封印改成链式哈希以支持 O(增量) 读取，属于事件/投影协议的破坏性变更，需要单独决策，未包含在上述任何 PR 中。**
