...
**补充：真实时序三点已补齐**（拆分文档 §6.4）。上表缺的是"快速路径刚写完、屏障尚未补投影"这一档，而它才是使用者实际遇到的时序。10,001 事件下走同一条 GUI 冷读路径：投影陈旧 **1647.9ms**、回合边界屏障后 **223.3ms**、投影被删除 **1846.0ms**。即**投影陈旧与投影缺失同量级**（两者走同一条回退），而屏障把这条路径降了 **7.4 倍**。残余窗口是两次屏障之间的冷读仍要 1.6–1.8s，只能靠"读取输入降为 O(增量)"那条独立条目消除。
