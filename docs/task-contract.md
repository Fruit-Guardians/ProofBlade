...
The workflow fixtures use `verification.kind: "hidden_scorer"` and require two reproduction records. Their visible target files are separate from `.proofblade/scorer.json`. A completion proposal stores a result hash and sensitive artifact reference; the event stream does not contain the result text. A successful command is valid only when it comes from the verifier lane, references an accepted completion and includes the required evidence records.
