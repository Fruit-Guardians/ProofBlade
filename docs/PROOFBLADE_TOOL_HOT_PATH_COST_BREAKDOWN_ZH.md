...
### 6.4 端到端核对：使用者报告的「秒级」确实落在重放回退上

父计划 §11 的完成定义里有一条中心判据：**几毫秒的普通工具不再被 ProofBlade 附加链路放大到秒级**，而 §7.2.1 把机制定位在重放回退（`control-store.ts:743`）。§5.7.2 的端到端数值当时只测了「投影存在 / 投影被删除」两点，缺的正是**「快速路径刚写完、屏障尚未补投影」**这一真实时序。补测如下（10,001 事件，全部 `persistProjection: false` 写入；每点都是全新 `DebugDataService` 冷缓存，走真实 GUI 读取路径）：
...
1. **「秒级」被复现，并且归因确定。** 快速路径刚写完时冷读要 **1648 ms**，投影被删除时 **1846 ms**——两者同量级，因为**投影陈旧与投影缺失走的是同一条回退路径**。这正好解释了使用者报告的现象：用户看到的秒级等待不是每次工具调用都慢（§7.2.1 已证明热路径 `read` 恒为 29–37 ms），而是**冷读恰好落在两次屏障之间**。
2. **回合边界屏障把这条路径从 1648 ms 降到 223 ms，7.4 倍**，且 `projection.json` 始终只有 4.5 KB。屏障的价值不在"让读取变快"，而在**不让读取掉进回退**。
3. **残余风险可量化：两次屏障之间的冷读仍要付 1.6–1.8 s。** 回合边界意味着一个长回合之内到达的 GUI 轮询仍可能落在陈旧窗口里。这不是本计划的缺陷，而是 §6.3 结论的另一面——屏障频率不该加密（加密到每 500 事件，冷读仍要 569 ms 且屏障累计开销线性上涨）。要真正消掉这个窗口，只能走 §6.3 末尾那条独立条目：把读取路径的输入从 O(历史) 降为 O(增量)。

**证据边界**：本表是本机实测（探针运行后已删除）。它证明的是**机制与量级**，不是"已达标"——§7.2.3 的目标段仍需按 §7.2.1 的基线评审，且本机数值不可直接当作阈值。
