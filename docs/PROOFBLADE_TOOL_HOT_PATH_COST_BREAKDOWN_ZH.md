...
4. **推论：修读取路径的 O(历史) 输入，会同时解决 §6.3、本节第 2 条和残余窗口。** 让读取只解析 `(投影 lastSeq, 流末]` 这一段，`snapshot()` 走投影路径就从 1877 ms 降到与新增事件数成正比。这就是 §6.3 末尾那条独立条目，也是本计划真正的剩余性能项。

**证据边界**：三张表都是本机实测（探针运行后已删除）。它证明的是**机制与量级**，不是"已达标"——§7.2.3 的目标段仍需按 §7.2.1 的基线评审，且本机数值不可直接当作阈值。

### 6.5 调查中发现：权威读取路径不做前缀封印校验（待维护者评估）

第 4 条那条优化本来受一个安全顾虑约束——"读取路径付 O(历史) 是为了重校验被篡改的事件前缀"。为确认这个顾虑是否成立，我做了篡改实验，结果与代码注释的描述**不一致**，需要维护者评估。
...
**代码路径**：`control-store.ts:744` 的 `if (!durableStateChanged && snapshot === undefined)` 导致 `loadProjection()` **只在本次读之前没有该 Run 的快照缓存时**才被调用。任何进程只要已经缓存过该 Run（列表轮询即可触发），revision 一变就跳过投影校验直接走 `replay()`（`:765`），而 `replay()` 只折叠磁盘上的事件，**不做任何前缀校验**。也就是说 `:739-741` 注释声称的"replay the full stream so modified prefixes and task-contract tampering are revalidated"，在**已缓存进程**这条路径上并未发生。

**我不主张这是漏洞**，理由如下，但正因如此需要你判断：

- 攻击前提是对 `events.jsonl` 有写权限；
- 我构造的是"文件长度不变的就地改写"，这**不是** append-only 违规，`revision()` 因 mtime 变化仍能察觉"有变化"，但没有任何一环把变化**判定为篡改**；
- `loadProjection()` 单独调用时能正确拒绝，说明封印本身有效——问题是权威读取路径没有走到它。

**待决问题**：这是有意的设计取舍（磁盘可写即视为可信，封印只用于跨进程防伪造），还是 `:744` 那个条件把重校验意外地变成了"仅冷启动"？如果是后者，最小修法是在 `durableStateChanged` 为真时也走一次 `loadProjection()` 校验（代价是回到 O(历史)，即 §6.3 那条张力），或在权威路径上引入独立的廉价完整性标记。

**证据边界**：以上全部为可复现实测，探针已删除、未入库。我**没有**添加测试来固化任一行为——现有行为是否正确尚未判定，把它写成断言会假装结论已经存在。
