...
1. **秒级不是回退造成的。** 投影陈旧时 `snapshot()` 走的是**投影 + `applyTail`** 路径，**不是** `eventStore.replay()`——加了计数器直接验证：该路径 `projectionHits=1, snapshotFallbacks=0`。§7.2.1 原先的"重放回退"归因是**推断，不是实测**，此处更正。
...
4. **推论：修读取路径的 O(历史) 输入，会同时解决 §6.3、本节第 2 条和残余窗口。** 让读取只解析 `(投影 lastSeq, 流末]` 这一段，`snapshot()` 走投影路径就从 1877 ms 降到与新增事件数成正比；前缀防篡改校验改在前缀 seal 的哈希上完成即可。这就是 §6.3 末尾那条独立条目，也是本计划真正的剩余性能项。
